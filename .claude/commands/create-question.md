...
1. **問題番号の自動採番**
   - **重要**: 既に存在しない最初の問題番号を自動検索する
   - 検索方法（シンプルな bash コマンドを使用）:
     ```bash
     # 既に存在する最大の問題番号を取得
     MAX_NUM=$(ls problems/*/question.md 2>/dev/null | sed 's|problems/||; s|/question.md||' | sort -n | tail -1)
     ...
     # その番号が既に存在する場合はスキップ（念のため）
     while [ -f "problems/$NEXT_NUM/question.md" ]; do
         NEXT_NUM=$(printf "%03d" $((10#$NEXT_NUM + 1)))
     done

     echo "$NEXT_NUM"
     ```
   - この方法により、既存の問題を上書きすることなく、常に新しい番号で問題を生成します
...
3. **問題の形式**
   問題は以下のMarkdown形式で `problems/NNN/question.md` に保存してください：
   - NNN は自動採番された番号（001, 002, ..., 999, 1000, ...）
   - ゼロパディングは最低3桁（001-999）、それ以上は桁数を増やす（1000, 1001, ...）
...
5. **確認事項（必ず実施すること）**
   ...
   **問題テーマと手牌の適合性チェック**:
   ...
   - ✅ **リーチ判断問題**の場合：
     ...
     - これらのパターンがない場合、純粋なリーチ判断問題（リーチするか黙聴か）
   - ✅ **手作り・手役選択問題**の場合：
     - イーシャンテン〜リャンシャンテンが適切
   - ✅ **待ち選択問題**の場合：
     - テンパイまたはイーシャンテン
...
**使用方法**:
- 引数なし: **既存の最大番号 + 1 の番号で自動生成**（例: 006 まである場合 → `problems/007/question.md` を生成）
- 引数あり: 指定した番号で生成（例: 引数に5を指定 → `problems/005/question.md`）
  - **注意**: 既に question が存在する場合でも上書きします

**重要な動作**:
- 自動採番により、既存の問題を誤って上書きすることを防ぎます
- 連続実行しても安全に新しい問題を生成できます
- 例: 006 まである状態で連続実行 → 007, 008, 009... と順次生成されます
