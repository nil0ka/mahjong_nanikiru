   ...
   - **押し引き判断における期待値分析**（押し引きテーマの問題では必須）:
     ...
     **重要: 押し引きの3つのグラデーション**
     ...
     1. **完全に降りる（ベタ降り）**:
        ...
        - 適用場面: オーラス・大差がついた状況・手が極端に遠い（3シャンテン以上）

     2. **様子見（中間戦略）** ← **実戦的に最も重要**:
        - 比較的安全な牌を切りつつ手を進める可能性を残す
        - リスク: 低〜中、リターン: 手が進めば攻めに転じる可能性
        - 適用場面: 親番1位・僅差・イーシャンテンで有効牌3種類程度
        - **比較的安全な牌**:
          - 孤立字牌（特に役牌でない場合や1枚のみの役牌）
          - 早巡に切られた牌の筋
          - 端牌（1,9）で河に見えている同種の牌が多い
        - **様子見の例**: 14枚 1m2m1p2p3p4p5p5s6s7s北北北發（親番1位、対面リーチ中）
          - 打發: 孤立牌で役にならず、字牌単騎の可能性は低い
          - 有効牌（3m, 3p, 6p）での進行可能性を残す
          - 發を切ることで1m,2m,1p,2p,4p,5pなどをツモると雀頭が形成されてテンパイ
          - 次巡で判断を変更できる柔軟性

     3. **全力で攻める**:
        - 危険牌でも手を進める牌を切る
        - リスク: 高、リターン: 和了可能性最大化
        - 適用場面: トップとの大差を詰める必要がある・手が良い

     **様子見戦略のポイント**:
     - 親番1位など有利なポジションでは、即座に完全降りする必要はない
     - 比較的安全な牌を切りつつ、次の1-2巡で状況を見極める
     - 手が進めば攻めに転じ、悪化すれば現物で降りる
     - **柔軟性が重要**: 状況に応じて戦略を変更できる選択肢を残す
