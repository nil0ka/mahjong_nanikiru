...
【問題生成の基本要件】
1. **難易度**: 1〜10の数値を選択（適切な難易度を選ぶこと）
   - 1-2 (Very Easy): 明確な正解が1つ、計算不要
   - 3-4 (Easy): 正解が明確、有力候補1-2個（例: 現物あり+明確な危険牌の押し引き）
...
2. テーマ: 問題のテーマを選択（例：リーチ判断、手役選択、押し引き、待ち選択、形式テンパイ、鳴き判断、安全牌選択など）
   - **CRITICAL リーチ判断問題の場合**:
     ...
     **パターン2: 暗刻+連続牌+対子（XXXYZWW）**:
     - **CRITICAL**: ノベタンより更に複雑で見落としやすい
     - 例: 4445688s東東東白白白 + ツモX
       - 索子部分 4445688s は**478s の3種待ち**（複合待ち）
       - 解釈A: 444（暗刻）+ 56（47両面待ち）+ 88（雀頭）→ 4sまたは7s待ち
       - 解釈B: 44（対子）+ 456（順子）+ 88（対子）→ 4sまたは8sシャボ待ち
       - 結果: 4sは両方、7sはAのみ、8sはBのみ → **4s, 7s, 8s全て当たり牌**
     - 他の例: 2223566, 5556788, 1112344
     - このパターンがある場合、問題は**複雑な待ち選択問題**になる
